...
/**
 * String类型的消费任务，提供两种消费方式：
 * 1, 批量消费，重载process(ConsumerRecords）
 * 2, 逐个消费，重载process(ConsumerRecord）
 */
@Slf4j
public class StringMessageProcessTask extends AbstractMessageProcessTask<String, String> {
    public StringMessageProcessTask(String taskId, Config config) {
        super(taskId, config);
    }
    ...
    protected void process(ConsumerRecords<String, String> records) {
        final Iterable<ConsumerRecord<String, String>> iterable = () -> records.iterator();
        final Stream<ConsumerRecord<String, String>> stream = StreamSupport.stream(iterable.spliterator(), false);
